...
async function openMeteoForecast(params: {
  ...
  const data = await (async () => {
    if (params.includeAir) {
      ...
      if (aRes && aRes.ok) {
        try {
          const aj = (await aRes.json()) as { current?: unknown }
          if (aj && aj.current) d.air_quality = aj.current
        } catch {
          /* Luftqualität ist optional */
        }
      }
      return d
    }
...
function readIncludeFlags(sp: URLSearchParams): { includeHourly: boolean; includeDaily: boolean; includeAir: boolean } {
  const includeHourly =
